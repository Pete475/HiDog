  ...
  const common: CheckItem[] = [
    {
      ...
      detail: "Work with a USDA-accredited vet familiar with Hawaii’s Dog and Cat Import Rules.",
    },
    {
      id: "rabies-docs",
      title: "Rabies documentation",
      detail: "Maintain records for two rabies vaccines given at least 30 days apart (when both dates apply).",
    },
    {
      id: "microchip-iso",
      title: "ISO microchip",
      detail: "Confirm microchip is implanted before or on first rabies vaccination date per your records.",
    },
    {
      id: "favn-lab",
      title: "FAVN test & lab routing",
      detail: "Blood draw sent to an HDOA-approved lab; track results and waiting periods.",
    },
  ];
  ...
  const hnl: CheckItem[] = [
    {
      ...
      detail: "Submit to HDOA with fees; this is the core state intake form for Honolulu (AQS) processing.",
    },
    {
      id: "state-payment",
      title: "State payment & confirmations",
      detail: "Pay required fees and keep receipts and approval correspondence for travel day.",
    },
    {
      id: "aqs-station",
      title: "Airport Animal Quarantine Station (AQS)",
      detail: "Coordinate arrival inspection and direct release through HNL’s AQS pathway.",
    },
    {
      id: "airline-cargo",
      title: "Airline booking & crate",
      detail: "Confirm airline pet policy, routing, and crate labeling for Hawaii entry.",
    },
  ];
  ...
  const neighbor: CheckItem[] = [
    {
      ...
      detail: "File neighbor-island intent paperwork on HDOA’s timeline—required in addition to core import docs.",
    },
    {
      id: "inspector-contract",
      title: "Contract a Neighbor Island inspector",
      detail: "Arrange a private vet contractor authorized for neighbor-island direct release inspections.",
    },
    {
      id: "permit-direct",
      title: "Permit for Direct Release",
      detail: "Secure neighbor-island permit requirements beyond HNL’s AQS-only flow.",
    },
    {
      id: "airline-neighbor",
      title: "Airline + island airport coordination",
      detail: "Confirm KOA, LIH, or OGG routing, inspection appointment, and cargo vs cabin rules.",
    },
  ];
...
export function printVetCheatSheet(): void {
  ...
  console.log("[HiDog] printVetCheatSheet — placeholder for vet summary export.");
  window.alert(
    ...
    >
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div>
            <h2 id="phase-3-heading" className="font-display text-xl font-semibold text-slatemuted-900">
              Phase 3 — The launchpad
            </h2>
            <p className="text-sm text-slatemuted-500">
              {state.neighborIsland
                ? "Neighbor island path: NOI, inspector, and permit steps are highlighted."
                : "Honolulu (HNL): AQS-279 and state payment are front and center."}
            </p>
          </div>
        </div>
        <button
        ...
        >
          ...
          <div>
            ...
            <p className="mt-1 leading-relaxed text-amber-900/90">
              Your airline must be notified of neighbor-island inspection requirements, and you should book a
              local accredited vet contractor <span className="font-semibold">at least 30 days in advance</span> of
              when you need them in the workflow. Missing this window can delay direct release.
            </p>
          </div>
        </div>
      )}
        ...
        {items.map((item, index) => (
          ...
          >
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-sm leading-relaxed text-slatemuted-600">{item.detail}</p>
            </div>
          </li>
        ))}
      </ul>
        ...
        <div className="mt-8 rounded-2xl border border-sand-200 bg-sand-50/50 p-4 text-xs leading-relaxed text-slatemuted-600">
          ...
          <ul className="mt-2 list-inside list-disc space-y-1">
            ...
            {timeline.earliestAfter30Days && (
              <li>30-day earliest arrival: {formatLong(timeline.earliestAfter30Days)}</li>
            )}
            {timeline.dateAfter120Days && (
              <li>120-day planning date (from FAVN): {formatLong(timeline.dateAfter120Days)}</li>
            )}
            {validation.isIneligible && <li className="text-red-700">Status: ineligible until dates are corrected.</li>}
          </ul>
        </div>
      )}
    </section>
